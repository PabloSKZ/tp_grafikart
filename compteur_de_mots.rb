text = "L'encre coule, le sang se répand ; la feuille buvard
...
text_arr.each { |k| k.chop! if k[-1,1] == "," }

compteur = {}
text_arr.each do |i| 
    compteur[i] = text_arr.count(i)
end

top = compteur.values.sort.reverse[0]

while top > 0
    puts top.to_s + " : " + compteur.map{ |k,v| v==top ? k : nil }.compact.to_s
    top -= 1
end